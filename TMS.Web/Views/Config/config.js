function ConfigSection_onEdit(e) {
    ...
    if (e.container.index() == 1) {
        jQuery("#FlagColor").data("kendoColorPicker");
    }
}

function ConfigSection_onSave(e) {
...
function ConfigSection_Error(gridName) {
    return function (args) {
        if (args.errors) {
            var grid = jQuery("#" + gridName).data("kendoGrid");
            grid.editable.element.find(".errors").html('');
            var validationTemplate = kendo.template(jQuery("#ConfigSectionErrorTemplate").html());
